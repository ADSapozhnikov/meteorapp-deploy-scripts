...
function setLogins {
    ...
    if [ -f ~/.aws/credentials ]; then {
        diff "${secretFolder}""/credentials" ~/.aws/credentials > "${tmpLog}"
        if [[ $(find "${tmpLog}" -type f -size +1c) ]]; then {
            rm -f ~/.aws/credentials
            cp "${secretFolder}""/credentials" ~/.aws
            echo ["$(date +"%d-%b %T %Z %z")"] "AWS login has been replaced" >> "${mainLog}"
        }
        fi
    }
    else {
        cp "${secretFolder}""/credentials" ~/.aws
        echo ["$(date +"%d-%b %T %Z %z")"] "AWS login has been copied" >> "${mainLog}"
    }
    fi
    ...
    if [[ $(find "${tmpLog}" -type f -size +1c) ]]; then {
        errorsCounter="$[$errorsCounter +1]"
        echo ["$(date +"%d-%b %T %Z %z")"] "AWS login has been checked with error" >> "${mainLog}"
    }
    else {
        echo ["$(date +"%d-%b %T %Z %z")"] "AWS login has been checked successfully" >> "${mainLog}"
    }
    fi
